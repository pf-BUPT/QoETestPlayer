...
public class VideoView extends SurfaceView implements MediaController.MediaPlayerControl {
    ...
    private OnCompletionListener mCompletionListener = new OnCompletionListener() {
        public void onCompletion(MediaPlayer mp) {
            ...
            if (mMediaController != null)
                mMediaController.hide();
            //结束时自动上传参数,在执行自定义onCompletion监听器之前执行

            if (mOnCompletionListener != null)
                mOnCompletionListener.onCompletion(mMediaPlayer);//用户自定义的结束监听器
        }
    };
        ...
        public boolean onInfo(MediaPlayer mp, int what, int extra) {
            ...
            if (mOnInfoListener != null) {
                ...
                if (what == MediaPlayer.MEDIA_INFO_BUFFERING_START) {
                    //Log.e("开始缓冲时的下载速度： "+extra);
                    startTime = System.currentTimeMillis();
                    if(videoBufferStart <0){
                        videoBufferStart = System.currentTimeMillis();
                    }
                } else if (what == MediaPlayer.MEDIA_INFO_BUFFERING_END) {
                    //Log.e("缓冲结束时下载速度： "+extra);
                    endTime = System.currentTimeMillis();
                    if (initime <= 0)
                        initime = endTime - startTime;
